...
class TableauExporter:
    ...
    def create_tvl_protocols_30d(self, tvl_file: str) -> str:
        ...
        try:
            # Load current data (all protocols)
            df_current = pd.read_csv(tvl_file)

            # Try to load historical data if available
            historical_file = get_processed_data_path("llama_tvl_protocols_30d.csv")
            df_historical = None
            if Path(historical_file).exists():
                df_historical = pd.read_csv(historical_file)
                print(f"Loaded historical data: {len(df_historical)} records")

            # Prepare current data snapshot
            tableau_columns = [
                "timestamp",
                "protocol_name",
                "protocol_slug",
            ...
            # Filter columns that exist
            available_columns = [
                col for col in tableau_columns if col in df_current.columns
            ]
            df_current_clean = df_current[available_columns].copy()

            # Add TVL in billions for readability
            if "tvl_usd" in df_current_clean.columns:
                df_current_clean["tvl_billion"] = (
                    df_current_clean["tvl_usd"] / 1e9
                ).round(2)

            # Ensure we have timestamp column (rename date if needed)
            if (
                "date" in df_current_clean.columns
                and "timestamp" not in df_current_clean.columns
            ):
                df_current_clean["timestamp"] = df_current_clean["date"]
            elif "timestamp" not in df_current_clean.columns:
                # If neither exists, create a default timestamp
                df_current_clean["timestamp"] = pd.Timestamp.now().strftime("%Y-%m-%d")

            print(f"Current data: Prepared {len(df_current_clean)} protocols")

            # Combine current and historical data
            if df_historical is not None and len(df_historical) > 0:
                # Add current data for all protocols
                df_combined = df_current_clean.copy()

                # Add historical data for protocols that have it
                df_combined["data_type"] = "current"
                df_historical["data_type"] = "historical"

                # Combine both datasets
                df_final = pd.concat([df_combined, df_historical], ignore_index=True)

                print(f"Combined data: {len(df_final)} total records")
                print(f"  - Current protocols: {len(df_current_clean)}")
                print(f"  - Historical records: {len(df_historical)}")

            else:
                df_final = df_current_clean
                df_final["data_type"] = "current"
                print(f"Using only current data: {len(df_final)} protocols")
